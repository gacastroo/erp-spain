...
public class DataImportService {
    ...
    public ImportPreview previewClients(MultipartFile file) {
        return preview(fileReader.read(file), safeFilename(file), CLIENT_HEADER_ALIASES, Map.of(
                LEGAL_NAME, "nombre_fiscal",
                TAX_ID, "nif_cif_nie"
        ));
    }

    public ImportPreview previewProducts(MultipartFile file) {
        return preview(fileReader.read(file), safeFilename(file), PRODUCT_HEADER_ALIASES, Map.of(
                NAME, "nombre",
                UNIT_PRICE, "precio"
        ));
    }
    ...
    public ImportResult importClients(MultipartFile file) {
        return importClients(fileReader.read(file));
    }

    public ImportResult importClients(String filename, byte[] content) {
        return importClients(fileReader.read(filename, content));
    }

    public ImportResult importProducts(MultipartFile file) {
        return importProducts(fileReader.read(file));
    }

    public ImportResult importProducts(String filename, byte[] content) {
        return importProducts(fileReader.read(filename, content));
    }

    private ImportResult importClients(TabularData data) {
    ...
    private ImportResult importProducts(TabularData data) {
    ...
    private ImportPreview preview(
        ...
        Map<String, Integer> columns = mapColumns(data.headers(), aliases);
        requireColumns(columns, required);

        Set<Integer> recognizedIndexes = new HashSet<>(columns.values());
        ...
        for (int index = 0; index < data.headers().size(); index++) {
            ...
            if (recognizedIndexes.contains(index)) {
        ...
        return new ImportPreview(
                ...
                recognizedHeaders,
                ignoredHeaders,
                data.rows().stream().limit(10).toList()
        );
    }

    private String safeFilename(MultipartFile file) {
        String filename = file == null ? null : file.getOriginalFilename();
        return filename == null || filename.isBlank() ? "archivo" : filename.trim();
    }
